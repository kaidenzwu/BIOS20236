...
def plotDenitSim(p,N0,A0, I0,tmin, tmax, n=1, tick_labels=True, axes_labels=True): ##KC added 6/21/21
    #print(p)
    lines = np.array([[0.000,0.447,0.741],[0.850,0.325,0.098]])
    plt.rcParams.update({"text.usetex": False, 'font.size': 16});
    
    y0 = np.append(N0, [A0, I0])
    #print('y0 is ' + str(y0))
    th = np.linspace(tmin,tmax,256)
    #print(np.min(th))
    #print(np.max(th))
#     y0 = np.append(experiment.N0, [experiment.A0, experiment.I0])
#     th = np.linspace(0,experiment.t[-1],256)
...
def residualGlob(p,experiments,n=1):
    ...
    try:
        ...
        #print('in the except')
        #print(experiments)
        res_out = np.append(res_out, residual(p, experiments, n))
        #print(res_out)
    return res_out
